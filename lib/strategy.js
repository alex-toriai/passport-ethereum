...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (!message) {
    return this.fail({message: 'Missing message.'}, 400);
  }
  if (!signature) {
    return this.fail({message: 'Missing signature.'}, 400);
  }
  ...
  this._store.verify(req, siweMessage.nonce, function(err, ok, info) {
    ...
    siweMessage.verify({signature})
      .then(function(response) {
        ...
        try {
          if (self._passReqToCallback) {
            self._verify(req, data.address, verified);
          } else {
            var arity = self._verify.length;
            switch (arity) {
              case 3:
                return self._verify(data.address, data.chainId, verified);
              default:
                return self._verify(data.address, verified);
            }
          }
        } catch (ex) {
      ...
      .catch(function(err) {
        return self.fail({message: err.error.type || 'Invalid signature.'}, 403);
      });
  });
};
